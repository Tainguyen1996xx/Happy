package com.example.modelfashion;

public class Constants {
    public static final String Key_collection_user = "users";
    public static final String Key_name = "name";
    public static final String Key_email = "emai;";
    public static final String Key_pass = "password";
    public static final String Key_Pre_name = "ChatApp";
    public static final String Key_SignEDname = "isSignedIn";
    public static final String Key_id_user = "userid";

    public static final String Key_FCM_TOKEN = "keyFcm";
    public static final String Key_User = "user";
    public static final String Key_collection_chat = "chat";
    public static final String Key_sender_id = "enderid";
    public static final String Key_recei_id = "receiid";
    public static final String Key_message = "message";
    public static final String Key_timestamp = "timestamp";
    public static final String Key_collection_convertsation = "convertsation";
    public static final String Key_sender_name = "sendername";
    public static final String Key_recei_name = "receiname";
    public static final String Key_last_message = "lastmess";
    public static final String Key_last_Avalibity = "valibity";
    public static final String REOMTE_MSG_AUT = "AUTHRIZATION";
    public static final String REOMTE_MSG_CONTENT_TYPE = "Conten-type";


}